...
func ParseInput(r *http.Request, input interface{}, supports fields) (err error) {
...
func valueOf(input interface{}) (inv reflect.Value, err error) {
	...
	if k != reflect.Pointer {
		err = errors.New("input is non pointer")
		return
	}
	inv = reflect.ValueOf(input).Elem()
	if !inv.IsValid() {
		err = errors.New("input is nil pointer")
		return
	}
	t = t.Elem()
	k = t.Kind()
	...
	if k != reflect.Pointer {
		err = errors.New("the type input point to is neither struct nor pointer")
		return
	}
	t = t.Elem()
	k = t.Kind()
	if k != reflect.Struct {
		err = errors.New("the pointer input point to is not point to struct")
		return
	}
	if inv.Elem().IsValid() {
		inv = inv.Elem()
...
func parseBody(r *http.Request, inv reflect.Value, supports fields) (err error) {
	...
	name := pft.Name
	supp := supports[name]
	requ := pft.Tag.Get("required") == "true"
	ptyp := pft.Tag.Get("type")
	if ptyp != "structure" {
		return
	}
	if !supp && r.ContentLength > 0 {
		err = ErrWithUnsupportedParam{name}
		return
	}
	if requ && r.ContentLength < 1 {
		err = ErrMissingRequiredParam{name}
		return
	}
