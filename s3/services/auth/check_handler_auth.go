...
func (s *AuthSys) CheckRequestAuthTypeCredential(ctx context.Context, r *http.Request, action s3action.Action, bucketName, objectName string) (cred auth.Credentials, owner bool, s3Err responses.Error) {
	...
	case AuthTypePresignedV2, AuthTypeSignedV2:
		if s3Err = s.IsReqAuthenticatedV2(r); s3Err != nil {
	...
	case AuthTypeSigned, AuthTypePresigned:
		...
		if s3Err = s.IsReqAuthenticated(ctx, r, region, ServiceS3); s3Err != nil {
			return cred, owner, s3Err
		}
	...
	if action == s3action.CreateBucketAction {
		...
		if s.PolicySys.bmSys.HasBucket(ctx, bucketName) {
			return cred, owner, responses.ErrBucketAlreadyExists
		}
	}
	...
	if bucketName == "" || action == s3action.CreateBucketAction {
		...
		if !s.PolicySys.bmSys.HasBucket(ctx, bucketName) {
			return cred, owner, responses.ErrNoSuchBucket
		}
	}

	return cred, owner, responses.ErrAccessDenied
}
...
func (s *AuthSys) IsReqAuthenticated(ctx context.Context, r *http.Request, region string, stype serviceType) (s3Error responses.Error) {
	...
	if err != nil {
		return responses.ErrInvalidDigest
	}
	...
	if err != nil {
		return responses.ErrInternalError
	}
	r.Body = reader
	return nil
}
...
func (s *AuthSys) IsPutActionAllowed(ctx context.Context, r *http.Request, action s3action.Action, bucketName, objectName string) (s3Err responses.Error) {
	...
	case AuthTypeUnknown:
		return responses.ErrSignatureVersionNotSupported
	case AuthTypeSignedV2, AuthTypePresignedV2:
		cred, owner, s3Err = s.getReqAccessKeyV2(r)
	...
	if !s.PolicySys.bmSys.HasBucket(ctx, bucketName) {
		return responses.ErrNoSuchBucket
	}
	return responses.ErrAccessDenied
}
